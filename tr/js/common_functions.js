(function ($) {
	...
	// Mobil menü overlay (layer) tıklanınca menüyü kapat
	$('.layer').on('click', function () {
		if ($('.panel_menu').hasClass('active')) {
			$('.panel_menu').removeClass('active');
	...
	// Carousel items centererd generals (excluding gallery)
	$('.carousel_item_centered:not(#gallery-carousel)').owlCarousel({    
	    loop:true,
	    margin:5,
	    nav:true,
	    dots:false,
	...
	// Gallery carousel - special configuration
	$('#gallery-carousel').owlCarousel({    
	    ...
	    onInitialized: function(event) {
	        // Apply center effect to middle item
	        setTimeout(function() {
	            applyGalleryCenterEffect();
	        }, 100);
	    },
	    onTranslated: function(event) {
	        // Reapply center effect after slide
	        applyGalleryCenterEffect();
	    }
	});
